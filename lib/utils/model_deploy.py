...
## freeze supervisor model while training
    if freeze_variable_func!= None:
        train_vars = freeze_variable_func()
    ...
    with tf.device(config.optimizer_device()):
        if optimizer:
            ...
            if freeze_variable_func != None:
                total_loss, clones_gradients = optimize_clones(clones, optimizer, var_list = train_vars)
            else:
            ...
            if clones_gradients:
                ...
                ##Mar 22 add gradient clip
                with tf.variable_scope('clip_gradient') as scope:
                    for i, (grad, var) in enumerate(clones_gradients):
                        clones_gradients[i] = (tf.clip_by_norm(grad, 1.0), var)
